...
/*15*/ {VFE_CMD_COLOR_COR_CFG, V32_COLOR_COR_LEN, V32_COLOR_COR_OFF,
	0xFF},
...
/*25*/ {VFE_CMD_CHROMA_SUBS_CFG, V32_CHROMA_SUBS_LEN,
	V32_CHROMA_SUBS_OFF,
	0xFF},
...
/*35*/ {VFE_CMD_MAIN_SCALER_UPDATE, V32_MAIN_SCALER_LEN,
	V32_MAIN_SCALER_OFF,
	0xFF},
...
/*55*/ {VFE_CMD_STATS_IHIST_UPDATE, V32_STATS_IHIST_LEN,
	V32_STATS_IHIST_OFF},
	{VFE_CMD_DUMMY_4},
	{VFE_CMD_EPOCH1_ACK},
	{VFE_CMD_EPOCH2_ACK},
...
/*95*/ {VFE_CMD_STATS_SKIN_STOP},
	...
	{VFE_CMD_SYNC_TIMER_SETTING, V32_SYNC_TIMER_LEN,
	 V32_SYNC_TIMER_OFF},
/*100*/ {VFE_CMD_ASYNC_TIMER_SETTING, V32_ASYNC_TIMER_LEN,
	 V32_ASYNC_TIMER_OFF},
	{VFE_CMD_LIVESHOT},
	{VFE_CMD_LA_SETUP},
...
static void vfe32_stop(void)
{
	...
	msm_io_w(VFE_DISABLE_ALL_IRQS, vfe32_ctrl->vfebase + VFE_IRQ_MASK_0);
	msm_io_w(VFE_DISABLE_ALL_IRQS, vfe32_ctrl->vfebase + VFE_IRQ_MASK_1);
...
/*[[ aswoogi_zsl*/
	v4l2_subdev_notify(&vfe32_ctrl->subdev,
			   NOTIFY_ISPIF_STREAM,
			   (void *)ISPIF_STREAM(RDI_0 | RDI_1 | PIX_0,
						ISPIF_OFF_IMMEDIATELY));
	/*v4l2_subdev_notify(&vfe32_ctrl->subdev,
	   NOTIFY_ISPIF_STREAM, (void *)ISPIF_STREAM(
	   RDI_1, ISPIF_OFF_IMMEDIATELY));
	   v4l2_subdev_notify(&vfe32_ctrl->subdev,
	   NOTIFY_ISPIF_STREAM, (void *)ISPIF_STREAM(
	   PIX_0, ISPIF_OFF_IMMEDIATELY)); */
/*]]*/
	/* axi halt command. */
	msm_io_w(AXI_HALT, vfe32_ctrl->vfebase + VFE_AXI_CMD);
	wmb();
	while (axiBusyFlag) {
	...
	/* Ensure the write order while writing
	   to the command register using the barrier */
	msm_io_w_mb(AXI_HALT_CLEAR, vfe32_ctrl->vfebase + VFE_AXI_CMD);

	/* after axi halt, then ok to apply global reset. */
	/* enable reset_ack and async timer interrupt only while
	   stopping the pipeline. */
	msm_io_w(0xf0000000, vfe32_ctrl->vfebase + VFE_IRQ_MASK_0);
...
static int vfe32_config_axi(int mode, uint32_t *ao)
{
	...
	vfe32_ctrl->outpath.out3.ch0 = 0x0000FFFF & *ch_info;
	vfe32_ctrl->outpath.out3.ch1 = 0x0000FFFF & (*ch_info++ >> 16);
	vfe32_ctrl->outpath.out3.ch2 = 0x0000FFFF & *ch_info++;
	...
	case OUTPUT_1_2_AND_3:
		...
		vfe32_ctrl->outpath.output_mode |= VFE32_OUTPUT_MODE_S;
		/* preview. */
		vfe32_ctrl->outpath.output_mode |= VFE32_OUTPUT_MODE_P;
		/* thumbnail. */
		vfe32_ctrl->outpath.output_mode |= VFE32_OUTPUT_MODE_T;
		break;

	case OUTPUT_1_AND_3:
		/* use wm0& 4 for preview, wm1&5 for video. */
		/* video */
		vfe32_ctrl->outpath.output_mode |= VFE32_OUTPUT_MODE_V;
		/* preview */
		vfe32_ctrl->outpath.output_mode |= VFE32_OUTPUT_MODE_PT;
		break;
...
static int vfe32_operation_config(uint32_t *cmd)
{
	...
	/*metadata  aswoogi_zsl */
	msm_io_w(*(++p), vfe32_ctrl->vfebase + VFE_RDI_CFG0);
...
static void vfe32_start_common(uint16_t operation_mode)
{
	...
	/* test: Qualcomm */
#ifndef QC_TEST
	CDBG("Hardcoding some VFE registers\n");
	/*msm_io_w(0x02D20788, vfe32_ctrl->vfebase + 0x01EC); */

	msm_io_setBits(vfe32_ctrl->vfebase, 0x004C, 0x2);
	msm_io_setBits(vfe32_ctrl->vfebase, 0x00AC, 0x2);
#endif
...
static int vfe32_zsl(void)
{
	...
	if ((vfe32_ctrl->operation_mode == VFE_MODE_OF_OPERATION_ZSL)) {
		...
		if (vfe32_ctrl->outpath.output_mode & VFE32_OUTPUT_MODE_V) {
			irq_comp_mask |=
			    ((0x1 << (vfe32_ctrl->outpath.out3.ch0 + 16)) |
			     (0x1 << (vfe32_ctrl->outpath.out3.ch1 + 16)));
		}
/*metadata*/
		if (vfe32_ctrl->jpeg_soc) {
			if (vfe32_ctrl->
			    outpath.output_mode & VFE32_OUTPUT_MODE_T) {
				irq_comp_mask |=
				    (0x1 << (vfe32_ctrl->outpath.out1.ch0 + 8));
			}
			if (vfe32_ctrl->
			    outpath.output_mode & VFE32_OUTPUT_MODE_S) {
				irq_comp_mask |=
			...
			if (vfe32_ctrl->
				...
				irq_comp_mask |=
				    ((0x1 << (vfe32_ctrl->outpath.out2.ch0 + 8))
				     | (0x1 <<
					(vfe32_ctrl->outpath.out2.ch1 + 8)));
			}
		}
		if (vfe32_ctrl->outpath.output_mode & VFE32_OUTPUT_MODE_P) {
			msm_io_w(1, vfe32_ctrl->vfebase +
				 vfe32_AXI_WM_CFG[vfe32_ctrl->outpath.
						  out0.ch0]);
			msm_io_w(1,
				 vfe32_ctrl->vfebase +
				 vfe32_AXI_WM_CFG[vfe32_ctrl->outpath.
...
#ifdef QC_TEST			/*aswoogi_zsl */
	vfe32_start_common(vfe32_ctrl->operation_mode);
#else
	vfe32_start_common();
#endif
		...
		if (vfe32_ctrl->outpath.output_mode & VFE32_OUTPUT_MODE_S) {
			...
			msm_io_w(1, vfe32_ctrl->vfebase +
				 vfe32_AXI_WM_CFG[vfe32_ctrl->outpath.
						  out1.ch0]);
#ifdef QC_TEST			/*aswoogi_zsl */
			msm_io_w(0x0000, vfe32_ctrl->vfebase +
				VFE_BUS_IO_FORMAT_CFG);/*npandit */
#else
...
static void vfe32_update(void)
{
	...
	if (vfe32_ctrl->update_la) {
		if (!msm_io_r(vfe32_ctrl->vfebase + V32_LA_OFF))
			msm_io_w(1, vfe32_ctrl->vfebase + V32_LA_OFF);
		else
			msm_io_w(0, vfe32_ctrl->vfebase + V32_LA_OFF);
...
static int vfe32_configure_pingpong_buffers(int id, int path)
{
	...
	if (outch->ping.ch_paddr[0] && outch->pong.ch_paddr[0]) {
		...
		CDBG("%s Configure ping/pong address for %d",
			__func__, path);
		vfe32_put_ch_ping_addr(outch->ch0, outch->ping.ch_paddr[0]);
		vfe32_put_ch_pong_addr(outch->ch0, outch->pong.ch_paddr[0]);
#ifdef QC_TEST			/*concurrent aswoogi_zsl */
		if (outch->ping.num_planes > 1)
			vfe32_put_ch_ping_addr(outch->ch1,
					       outch->ping.ch_paddr[1]);
		if (outch->pong.num_planes > 1)
...
static int vfe32_proc_general(struct msm_isp_cmd *cmd)
{
	...
	case VFE_CMD_STATS_AE_START:{
			...
			old_val =
			    msm_io_r(vfe32_ctrl->vfebase + VFE_MODULE_CFG);
			old_val |= AE_BG_ENABLE_MASK;
			msm_io_w(old_val, vfe32_ctrl->vfebase + VFE_MODULE_CFG);
			msm_io_memcpy(vfe32_ctrl->vfebase +
				      vfe32_cmd[cmd->id].offset, cmdp,
				      (vfe32_cmd[cmd->id].length));
		}
		break;
	...
	case VFE_CMD_MCE_CFG:{
			cmdp = kmalloc(cmd->length, GFP_ATOMIC);
			/* Incrementing with 4 so as to point to
			the 2nd Register as
			...
			cmdp_local += 1;
			msm_io_memcpy(vfe32_ctrl->vfebase +
				      vfe32_cmd[cmd->id].offset, cmdp_local,
				      (vfe32_cmd[cmd->id].length));
		}
		break;
	...
	case VFE_CMD_MESH_ROLL_OFF_CFG:{
			...
			for (i = 0; i < (V32_MESH_ROLL_OFF_INIT_TABLE_SIZE * 2);
			     i++) {
				msm_io_w(*cmdp_local,
			...
			for (i = 0;
			     i < (V32_MESH_ROLL_OFF_DELTA_TABLE_SIZE * 2);
			     i++) {
				msm_io_w(*cmdp_local,
					 vfe32_ctrl->vfebase + VFE_DMI_DATA_LO);
				cmdp_local++;
	...
	case VFE_CMD_GET_MESH_ROLLOFF_TABLE:
		temp1 = sizeof(uint32_t) * ((V32_MESH_ROLL_OFF_INIT_TABLE_SIZE *
					     2) +
					    (V32_MESH_ROLL_OFF_DELTA_TABLE_SIZE
					     * 2));
		if (cmd->length != temp1) {
			rc = -EINVAL;
			goto proc_general_done;
	...
	case VFE_CMD_DEMOSAICV3_ABF_CFG:{
			...
			*cmdp_local = new_val;
			...
			msm_io_memcpy(vfe32_ctrl->vfebase +
				      V32_DEMOSAICV3_0_OFF, cmdp_local, 4);
	...
	case VFE_CMD_GET_RGB_G_TABLE:
		...
		old_val = msm_io_r(vfe32_ctrl->vfebase + V32_RGB_G_OFF);
		temp2 = old_val ? RGBLUT_RAM_CH0_BANK1 : RGBLUT_RAM_CH0_BANK0;
		for (i = 0; i < 3; i++) {
			vfe32_read_gamma_cfg(temp2,
					     cmdp_local +
					     (VFE32_GAMMA_NUM_ENTRIES * i));
			temp2 += 2;
		}
		if (copy_to_user((void __user *)(cmd->value), cmdp, temp1)) {
			rc = -EFAULT;
...
#ifdef QC_TEST			/*aswoogi_zsl*/
			*(cmdp_local + 1) =
			    msm_io_r(vfe32_ctrl->vfebase + VFE33_DMI_DATA_HI);
			*cmdp_local = msm_io_r(vfe32_ctrl->vfebase +
					       VFE33_DMI_DATA_LO);
#else
			*cmdp_local = msm_io_r(vfe32_ctrl->vfebase +
	...
	case VFE_CMD_GET_REG_DUMP:
		...
		msm_io_dump(vfe32_ctrl->vfebase,
			    vfe32_ctrl->register_total * 4);
		CDBG("%s: %p %p %d\n", __func__, (void *)cmdp,
...
static void vfe32_process_reg_update_irq(void)
{
	uint32_t old_val;
	unsigned long flags;
	struct vfe32_output_ch *video_ch;

	CDBG("%s: jpeg_soc = %d\n", __func__, vfe32_ctrl->jpeg_soc);
	if (vfe32_ctrl->jpeg_soc)
		video_ch = &vfe32_ctrl->outpath.out3;
	else
	...
	if (vfe32_ctrl->recording_state ==
		...
		if (vfe32_ctrl->outpath.output_mode & VFE32_OUTPUT_MODE_V) {
			CDBG("%s: %d\n", __func__, __LINE__);
			msm_io_w(1, vfe32_ctrl->vfebase +
				 vfe32_AXI_WM_CFG[video_ch->ch0]);
			msm_io_w(1, vfe32_ctrl->vfebase +
				 vfe32_AXI_WM_CFG[video_ch->ch1]);
		}
	...
	if (vfe32_ctrl->start_ack_pending == TRUE) {
		...
		if (vfe32_ctrl->update_ack_pending == TRUE) {
			vfe32_ctrl->update_ack_pending = FALSE;
			spin_unlock_irqrestore(&vfe32_ctrl->update_ack_lock,
					       flags);
...
{
	if (rdi == RDI_0) {
		vfe32_ctrl->rdi0FrameId++;
		if (vfe32_ctrl->rdi0FrameId == 0)
			vfe32_ctrl->rdi0FrameId = 1;
	} else if (rdi == RDI_1) {
...
static void vfe32_process_camif_sof_irq(void)
{
	...
	if (vfe32_ctrl->operation_mode == VFE_MODE_OF_OPERATION_RAW_SNAPSHOT) {
...
static void vfe32_process_error_irq(uint32_t errStatus)
{
	...
	if (errStatus & VFE32_IMASK_VIOLATION) {
		pr_err_ratelimited("vfe32_irq: violation interrupt,"
		       " value = %x\n",
		       msm_io_r(vfe32_ctrl->vfebase + 0x7B4));
	}
...
static void vfe_send_outmsg(struct v4l2_subdev *sd, uint8_t msgid,
			    ...
			    uint32_t ch2_paddr, uint32_t frame_id)
{
...
static bool vfe32_is_free_buf_avail(int path)
{
	struct msm_sync *p_sync = (struct msm_sync *)vfe_syncdata;
	struct msm_cam_v4l2_device *pcam = p_sync->pcam_sync;
	return (msm_mctl_check_free_buf(&pcam->mctl, path) >= 0);
}
...
static void vfe32_process_zsl_frame(void)
{
	...
	/* Thumbnail  - RDI1 */
	free_buf_t = vfe32_check_free_buffer(VFE_MSG_OUTPUT_IRQ,
					   VFE_MSG_OUTPUT_T);
	if (free_buf_t) {
		ch0_paddr = vfe32_get_ch_addr(ping_pong,
		...
		CDBG("thumbnail get ch0_paddr = 0x%x", ch0_paddr);
		CDBG("thumbnail get ch1_paddr = 0x%x", ch1_paddr);
		CDBG("thumbnail get ch2_paddr = 0x%x", ch2_paddr);

		vfe32_put_ch_addr(ping_pong,
				  vfe32_ctrl->outpath.out1.ch0,
				  free_buf_t->ch_paddr[0]);
		if (free_buf_t->num_planes > 1)
			vfe32_put_ch_addr(ping_pong,
					  vfe32_ctrl->outpath.out1.ch1,
					  free_buf_t->ch_paddr[1]);
		if (free_buf_t->num_planes > 2)
			vfe32_put_ch_addr(ping_pong,
					  vfe32_ctrl->outpath.out1.ch2,
					  free_buf_t->ch_paddr[2]);
		CDBG("thumbnail put ch0_paddr = 0x%x", free_buf_t->ch_paddr[0]);
		CDBG("thumbnail put ch1_paddr = 0x%x", free_buf_t->ch_paddr[1]);
		CDBG("thumbnail put ch2_paddr = 0x%x", free_buf_t->ch_paddr[2]);
	...
	if (free_buf_s) {
		...
		CDBG("mainimg get ch0_paddr = 0x%x", ch0_paddr);
		CDBG("mainimg get ch1_paddr = 0x%x", ch1_paddr);
		CDBG("mainimg get ch2_paddr = 0x%x", ch2_paddr);

		vfe32_put_ch_addr(ping_pong,
				  vfe32_ctrl->outpath.out2.ch0,
				  free_buf_s->ch_paddr[0]);
		if (free_buf_s->num_planes > 1)
			vfe32_put_ch_addr(ping_pong,
					  vfe32_ctrl->outpath.out2.ch1,
					  free_buf_s->ch_paddr[1]);
		if (free_buf_s->num_planes > 2)
			vfe32_put_ch_addr(ping_pong,
					  vfe32_ctrl->outpath.out2.ch2,
...
static void vfe32_process_output_path_irq_2(void)
{
	...
	if (free_buf) {
		...
		/* Y channel */
		ch0_paddr = vfe32_get_ch_addr(ping_pong, video_ch->ch0);
		/* Chroma channel */
		ch1_paddr = vfe32_get_ch_addr(ping_pong, video_ch->ch1);
		ch2_paddr = vfe32_get_ch_addr(ping_pong, video_ch->ch2);

		/* Y channel */
		vfe32_put_ch_addr(ping_pong,
				  video_ch->ch0, free_buf->ch_paddr[0]);
		/* Chroma channel */
		vfe32_put_ch_addr(ping_pong,
				  video_ch->ch1, free_buf->ch_paddr[1]);
		if (free_buf->num_planes > 2)
			vfe32_put_ch_addr(ping_pong,
					  video_ch->ch2, free_buf->ch_paddr[2]);

		vfe_send_outmsg(&vfe32_ctrl->subdev,
				MSG_ID_OUTPUT_V, ch0_paddr,
				ch1_paddr, ch2_paddr, vfe32_ctrl->vfeFrameId);
					       ...
					       uint32_t newAddr)
{
	...
	pingpongStatus =
	    ((msm_io_r(vfe32_ctrl->vfebase + VFE_BUS_PING_PONG_STATUS))
...
static void vfe_send_stats_msg(uint32_t bufAddress, uint32_t statsNum)
{
	...
	case statsAeNum:{
			...
			spin_unlock_irqrestore(&vfe32_ctrl->aec_ack_lock,
					       flags);
		}
		break;
	case statsAfNum:{
	...
	case statsAwbNum:{
			...
			spin_unlock_irqrestore(&vfe32_ctrl->awb_ack_lock,
					       flags);
		}
		break;
	...
	case statsIhistNum:{
			...
			spin_unlock_irqrestore(&vfe32_ctrl->ihist_ack_lock,
					       flags);
		}
		break;
	case statsRsNum:{
			...
			spin_unlock_irqrestore(&vfe32_ctrl->rs_ack_lock,
					       flags);
		}
		break;
	case statsCsNum:{
			...
			spin_unlock_irqrestore(&vfe32_ctrl->cs_ack_lock,
					       flags);
		}
		break;
...
static void vfe32_process_stats_ae_irq(void)
{
	...
	if (!(vfe32_ctrl->aecStatsControl.ackPending)) {
		...
		pr_err("%s: droppedStatsFrameCount =%d", __func__, vfe32_ctrl->aecStatsControl.droppedStatsFrameCount+1);
		spin_unlock_irqrestore(&vfe32_ctrl->aec_ack_lock, flags);
...
static void vfe32_process_stats_awb_irq(void)
{
	...
	if (!(vfe32_ctrl->awbStatsControl.ackPending)) {
		...
		pr_err("%s: droppedStatsFrameCount =%d", __func__, vfe32_ctrl->awbStatsControl.droppedStatsFrameCount+1);
		spin_unlock_irqrestore(&vfe32_ctrl->awb_ack_lock, flags);
...
static void vfe32_process_stats_ihist_irq(void)
{
	...
	if (!(vfe32_ctrl->ihistStatsControl.ackPending)) {
		...
		spin_unlock_irqrestore(&vfe32_ctrl->ihist_ack_lock, flags);
		pr_err("%s: droppedStatsFrameCount =%d", __func__, vfe32_ctrl->ihistStatsControl.droppedStatsFrameCount+1);
...
static void vfe32_process_stats_rs_irq(void)
{
	...
	if (!(vfe32_ctrl->rsStatsControl.ackPending)) {
		...
		spin_unlock_irqrestore(&vfe32_ctrl->rs_ack_lock, flags);
		pr_err("%s: droppedStatsFrameCount =%d", __func__, vfe32_ctrl->rsStatsControl.droppedStatsFrameCount+1);
...
static void vfe32_process_stats_cs_irq(void)
{
	...
	if (!(vfe32_ctrl->csStatsControl.ackPending)) {
		...
		spin_unlock_irqrestore(&vfe32_ctrl->cs_ack_lock, flags);
		pr_err("%s: droppedStatsFrameCount =%d", __func__, vfe32_ctrl->csStatsControl.droppedStatsFrameCount+1);
...
static irqreturn_t vfe32_parse_irq(int irq_num, void *data)
{
	...
	qcmd = kzalloc(sizeof(struct vfe32_isr_queue_cmd), GFP_ATOMIC);
	if (!qcmd) {
	...
	if ((qcmd->vfeInterruptStatus1 & 0x3FFF00) &&
		...
		msm_io_w(0x0, vfe32_ctrl->vfebase + VFE_IRQ_MASK_0);
		...
		msm_io_w(0x2, vfe32_ctrl->vfebase +
			VFE_CAMIF_COMMAND);
		msm_io_w(AXI_HALT,
				 ...
				 unsigned int subdev_cmd, void *arg)
{
	...
	if ((cmd->cmd_type == CMD_STATS_AF_ENABLE) ||
		...
		scfg = kmalloc(sizeof(struct vfe_cmd_stats_buf), GFP_ATOMIC);
		if (!scfg) {
			rc = -ENOMEM;
			goto vfe32_config_done;
		}
